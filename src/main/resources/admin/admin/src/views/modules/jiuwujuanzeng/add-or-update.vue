  ...
  data() {
	...
	yonghushouji : false,
      ...
      rules: {
          ...
          juanzengwupin: [
                { required: true, message: '捐赠物品不能为空', trigger: 'blur' },
          ],
          juanzengshijian: [
          ],
          yonghuzhanghao: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiuwujuanzeng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiuwujuanzengCrossAddOrUpdateFlag = false;
						 this.parent.search();
