...
export default {
  ...
  methods: {
	...
	this.ruleForm.xinwenneirong = this.ruleForm.xinwenneirong.replace(reg,'../../../springboot60h87/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuanchuanxinwen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuanchuanxinwenCrossAddOrUpdateFlag = false;
						 this.parent.search();
